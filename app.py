...
if st.session_state.feed_data:
    ...
    with left_col:
        search = st.text_input("Buscar", placeholder="agentes, RAG, memory, supply chain...", label_visibility="collapsed")
            ...
            for cat, ci in grouped.items():
                for item in ci:
                    if query in item["title"].lower() or query in item["summary"].lower():
                        dg[cat].append(item)
            st.caption(f"{sum(len(v) for v in dg.values())} resultados para '{query}'")
        else:
            dg = grouped
            st.caption(f"{len(feed_items)} items cargados")
...
<div class="card-summary">{item['summary'][:200]}{'...' if len(item['summary']) > 200 else ''}</div>
                    ...
                    if st.button("→ Ver", key=f"btn_{item['_idx']}"):
    ...
    with right_col:
        if st.session_state.selected_item:
            ...
            if sel.get("link"):
                st.markdown(f'[🔗 Abrir original]({sel["link"]})')
            st.markdown("---")
            ...
            st.markdown('<div class="section-header">🧠 Análisis global</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="analysis-box">{st.session_state.analysis}</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="empty-panel">← cargá las fuentes para empezar</div>', unsafe_allow_html=True)
